function [x,y,s, optimal_solution] = ipm_matrix_free_driver_dynamic_reg(c,A,b,Q,k,maxit_ipm,maxit_kryl,printlevel,Rd_max,Rd_min,gamma)
...
%  Author: Angelis Tzouchas
  ...
  while (iter < maxit_ipm )
    ...
    theta = x.*(1./s);
    inv_theta = 1./theta;
    Q_tilde = inv_theta + gamma*gamma;
    
    
    % inv_Q_tilde is a vector
    inv_Q_tilde = (1./Q_tilde);
          
    rhs_of_pcg = h_double_dash + A*(inv_Q_tilde.*f_double_dash);
    
    GR_operator_pcd = @(x) (A*(inv_Q_tilde.*(A'*x)));
    ...
    PS = partial_cholesky_dynamic_reg(k,@(x) GR_operator_pcd(x),d,Rd_max,Rd_min);    
    
    % CHECK THE RD vector!
    Rd_matrix_dynamic = Rd_max*(PS.indicator);
    % check the for loop
    for i = 1: length(PS.indicator)
        if PS.indicator(i) == 0
            Rd_matrix_dynamic(i) = Rd_min;
        end
    end
        
    GR_operator = @(x) (A*(inv_Q_tilde.*(A'*x))+diag(Rd_matrix_dynamic)*x);
