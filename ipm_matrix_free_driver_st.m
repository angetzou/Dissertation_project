function [x,y,s, optimal_solution] = ipm_matrix_free_driver_st(c,A,b,Q,k,maxit_ipm,maxit_kryl,printlevel,gamma,delta)
...
%  Author: Angelis Tzouchas
  ...
  %[x,y,s] = starting_point2(c,A,b);
  [x,y,s] = starting_point(c,A,b);
  mu = 1;
  ...
  while (iter < maxit_ipm )
    ...
    NS = partial_cholesky_decomposition(k,@(x) GR_operator(x),d);  
    
    [dy, flag,rel_res,Kryl_it] = pcg(@(x) GR_operator(x),rhs_of_pcg,ekm ,maxit_kryl,@(x) inverse_preconditioner(x,NS));
    flag
    rel_res
    Kryl_it
